...
function BarPanel({
  ...
  height?: number
  /**
   * When true, the chart is rendered at its full intrinsic height inside an
   * overflow-y: auto wrapper of `visibleHeight` pixels. Used by the Stage
   * panel so all 15 stages remain visible (scrollable) without making the
      ...
      {hasData ? (
        scrollable ? (
          <div
            ...
            style={{ maxHeight: visibleHeight ?? 260 }}
          >
            {chart}
          </div>
        ) : (
...
export default function DashboardClient({ companies, signals }: Props) {
  ...
  // Show every canonical stage so the user can see the full pipeline
  // structure (Bootstrapped → Pre-Seed → … → IPO → Acquired) even when
  // most are empty. The card itself is scrollable so it doesn't stretch to
  // 15 × 32 = 480px tall.
  ...
  return (
    <div className="px-8 py-6 max-w-6xl">
      ...
      {/* Pipeline + Stage. items-start so the cards size to their own content
          (the Status card is naturally short; the Stage card is fixed-height
          + scrollable so it stays compact even with 15 stages). */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6 items-start">
        ...
        <BarPanel
          ...
          data={stageData}
          filterKey="stage"
          // Render the full chart at ~32px per stage (covers all 15 even
          // when most are empty), but cap the visible card area so the user
          // scrolls inside the Stage card instead of the whole dashboard.
          scrollable
          height={stageData.length * 32}
          visibleHeight={260}
        />
      </div>
